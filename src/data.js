...
// 怪物依自己的等級小幅加成，彌補玩家裝備/屬性點越疊越多造成的差距——
// 等級越高的怪物越靠近王座，玩家推到那裡之前多半已經練了不少級、撿了
// 不少裝備，用平方根曲線（先快後緩，弧度上升）補一個倍率，讓「越硬的
// 怪」相對再硬一點；封頂 +50%，不會蓋過裝備/等級成長本身，純粹是拉住
// 後期差距別拉太開。改成跟等級掛鉤（不是回合數）：同一等級的怪物，
// 不管第幾回合遇到、重生幾次，倍率都固定一樣，纯粹取決於牠多硬。
const MON_SCALE_CAP = 1.5, MON_SCALE_K = 0.12;
function monScaleFor(lv) {
  return Math.min(MON_SCALE_CAP, 1 + MON_SCALE_K * Math.sqrt(Math.max(0, lv - 1)));
}
...
// 18 是等級上限還是 10 時調的；等級上限拉到 30 之後（LV_GAIN.def 加上
// 60 點自由分配、裝備、轉職加成疊起來）防禦動輒 60~90，減傷一度飆到
// 76~84%。中間試過 40（60~70%），使用者最後拍板要更平的曲線，改成
// 100：同樣防禦範圍落在 29~49%，防禦不再是後期近乎無敵的手段。
const MIT_K = 100;
...
const LV_GAIN = { hp: 10, atk: 2, def: 1 };
...
const PROMO_LV = 5, PROMO = { hp: 20, atk: 3, def: 2, mov: 1 };
